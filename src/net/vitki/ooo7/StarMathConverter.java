...
public final class StarMathConverter {
	...
	public static boolean MEDIAWIKI_MODE = true;
	...
	StarMathConverter(I18n i18n, Hashtable cs, boolean uc){
        this.i18n = i18n;
        configSymbols = cs;
        bUseColor = uc;
    }
        ...
        if (buffer.peekChar()=='\0'){ // End of input
            ...
            if (MEDIAWIKI_MODE) {
            	if (sText.equals("\u2032") || sText.equals("'")) {
                    curToken.assign(Token.SPECIAL, "\\prime", 5);
            		return;
            	}
            	if (sText.equals("\u2032\u2032") || sText.equals("''")) {
                    curToken.assign(Token.SPECIAL, "\\prime\\prime", 5);
            		return;
            	}
            	if (sText.equals("*")) {
                    curToken.assign(Token.SPECIAL, "*", 5); // NB: maybe \star is better ?
            		return;
            	}
            }
    ...
    private String align(float fSize, Token eAlign,boolean bUseAlignment,boolean bNeedNull){
    	...
    	if (MEDIAWIKI_MODE) // Tex of MediaWiki doesn't support "hfill"
    		bUseAlignment = false;
    ...
    private String subsup(float fSize, Token eAlign,String sBody, TGroup eActiveGroup){
        ...
        if (eActiveGroup==TGroup.LIMIT){
            ...
            if (sLsub==null && sLsup==null && sCsub==null && sCsup==null){
                ...
                // scripts above/below
            	if (MEDIAWIKI_MODE) {
            		// TEX module in MediaWiki doesn't understand \\overset or \\underset
            		String ssBody = sBody;
            		if (ssBody.startsWith("\\mathit"))
            			ssBody = "\\bold"+ssBody.substring(7);
            		if (sCsub==null){
            			return "\\begin{matrix} "+sCsup+" \\\\ "+ssBody+" \\\\\\ {} \\end{matrix}";
            		}
            		else if (sCsup==null){
            			return "\\begin{matrix} {} \\\\ "+ssBody+" \\\\ "+sCsub+" \\end{matrix}";
            		}
            		else {
            			return "\\begin{matrix} "+sCsup+" \\\\ "+ssBody+" \\\\ "+sCsub+" \\end{matrix}";
            		}
            	} else {
            		if (sCsub==null){
    ...
    private String attributes(float fSize, Token eAlign){
        ...
        if (curToken.eType==Token.FONT){
            ...
            if (MEDIAWIKI_MODE) {
            	if (curToken.eType == Token.TEXT) {
            		// This is usually used with "/" to make a wideslash.
            		// Since extending the operator parser is going be too painful,
            		// I'm adding this as a hack here. Hence, no checking at all.
            		String sOp = curToken.sLaTeX;
            		String sSizer = (fSize > 20 ? "\\Big " : "");
            		nextToken();
            		return "{"+sSizer+sOp+"}";
            	}
            }
